# Sample traject configuration.  This is really minimal, and just intended
# at this point to give the flavour of things.
#
# Usage: bundle exec traject -c config/ncsu_sample.rb [list of paths to MARC files]

# You will probably want to customize how the 'id' is generated, since
# that depends on where you put your institutional control number.
#
# NCSU puts its local identifiers in 918$a, and prepends "NCSU"
# to the identifier found there for use as a unique identifier within the
# index.  Search the file for 'NCSU' to find where this is configured.
#
# Elsewhere, you're on your own (for now).
...
# This is a *slight* customized version of the demo configuration
#  file from the tranject project, modified just enough to get NCSU's
...
to_field "id", extract_marc("918a", :first => true) do |marc_record, accumulator, context|
  accumulator.collect! {|s| "NCSU#{s}"}
end
...
#to_field "format",              marc_formats[0]
...
to_field "author_display",      extract_marc("100abcdq:110:111", :first=> true)
